...
export default function ReceivablesScreen() {
  ...
  // Tab state
  const [activeTab, setActiveTab] = useState<"DASHBOARD" | "AGING" | "REPORTS">(
    "DASHBOARD",
  );
  ...
  const [selectedCustomer, setSelectedCustomer] =
    useState<CustomerAgingType | null>(null);
  ...
  const [billSettlements, setBillSettlements] = useState<
    Record<string, BillSettlementType[]>
  >({});
    ...
    try {
      ...
      if (statsJson.success && statsJson.stats) {
        setStats({
          ...
          totalCustomersWithCredit: Number(
            statsJson.stats.totalCustomersWithCredit || 0,
          ),
          collectionsToday: Number(statsJson.stats.collectionsToday || 0),
          collectionsThisMonth: Number(
            statsJson.stats.collectionsThisMonth || 0,
          ),
  ...
  const handleToggleBillExpansion = async (invoiceTransactionId: string) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/credit-customers/invoice-settlements/${invoiceTransactionId}`,
        {
  ...
  const fetchCustomerDetails = async (customer: CustomerAgingType) => {
    ...
    try {
      ...
      const ledgerRes = await fetch(
        `${API_URL}/api/credit-customers/statement/${customer.MemberId}`,
        {
          headers: { Authorization: token ? `Bearer ${token}` : "" },
        },
      );
      ...
      const billsRes = await fetch(
        `${API_URL}/api/credit-customers/outstanding/${customer.MemberId}`,
        {
          headers: { Authorization: token ? `Bearer ${token}` : "" },
        },
      );
      ...
      if (billsJson.success) {
        const parsedBills = (billsJson.outstandingBills || []).map(
          (b: any) => ({
            ...b,
  ...
  const handleSendWhatsAppReminder = () => {
    ...
    const phoneWithCountry = hasPlus
      ? cleanPhone
    ...
    router.push({
      ...
      params: {
        ...
        allocations: JSON.stringify([
          { settlementId: bill.SettlementId, amount: bill.OutstandingAmount },
        ]),
      },
    });
  };
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={["top"]}>
        ...
        <View style={styles.headerBar}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.screenTitle}>Credit Management</Text>
            <Text style={styles.screenSubtitle}>
              Ledger-based accounting & collections
            </Text>
          </View>
          <TouchableOpacity
          ...
          >
            ...
            <Text
              style={{ color: "#fff", fontFamily: Fonts.bold, fontSize: 13 }}
            >
              Add Customer
            </Text>
          </TouchableOpacity>
          ...
          {isMobile ? (
            ...
            >
              ...
              >
                ...
                >
                  <Text style={[styles.kpiLabel, { color: Theme.danger }]}>
                    OVERDUE (30+ DAYS)
                  </Text>
                ...
                <Text style={[styles.kpiVal, { color: Theme.danger }]}>
                  {currencySymbol}
                  {(stats?.totalOverdue || 0).toFixed(2)}
                </Text>
                <Text
                  style={{
                    fontSize: 10,
              ...
              >
                ...
                >
                  <Text style={[styles.kpiLabel, { color: Theme.success }]}>
                ...
                <Text style={[styles.kpiVal, { color: Theme.success }]}>
                  {currencySymbol}
                  {(stats?.collectionsThisMonth || 0).toFixed(2)}
                </Text>
                <Text
                  style={{
                    fontSize: 10,
                ...
                >
                  Today: {currencySymbol}
                  {(stats?.collectionsToday || 0).toFixed(2)}
                </Text>
              </View>
            </ScrollView>
          ) : (
            ...
            <View style={styles.listContainer}>
              {filteredCustomers.length === 0 ? (
                ...
                filteredCustomers.map((item) => (
                  <View key={item.MemberId} style={styles.customerCard}>
                    <TouchableOpacity
                      activeOpacity={0.8}
                  ...
                  <View key={item.MemberId} style={styles.agingCard}>
                    ...
                    >
                      ...
                      <View style={{ alignItems: "flex-end" }}>
                        ...
                        <Text
                          style={[styles.outstandingValue, { fontSize: 16 }]}
                        >
                          {currencySymbol}
                          {item.OutstandingBalance.toFixed(2)}
                        </Text>
                    ...
                    <View style={styles.agingGrid}>
                      <View style={styles.agingGridItem}>
                        ...
                        <Text
                          style={[
                            styles.agingGridValue,
                            item.Bucket0to30 > 0 && { color: Theme.success },
                          ]}
                        >
                          {currencySymbol}
                          {item.Bucket0to30.toFixed(0)}
                        </Text>
                      </View>
                      <View style={styles.agingGridItem}>
                        ...
                        <Text
                          style={[
                            styles.agingGridValue,
                            item.Bucket31to60 > 0 && { color: Theme.warning },
                          ]}
                        >
                          {currencySymbol}
                          {item.Bucket31to60.toFixed(0)}
                        </Text>
                      </View>
                      <View style={styles.agingGridItem}>
                        ...
                        <Text
                          style={[
                            styles.agingGridValue,
                            item.Bucket61to90 > 0 && { color: Theme.primary },
                          ]}
                        >
                          {currencySymbol}
                          {item.Bucket61to90.toFixed(0)}
                        </Text>
                      </View>
                      <View style={styles.agingGridItem}>
            ...
            <View style={styles.listContainer}>
              <View style={styles.reportSummaryCard}>
                ...
                <View style={styles.reportSummaryGrid}>
                  <View style={styles.reportSummaryItem}>
                    ...
                    <Text
                      style={[styles.reportSummaryVal, { color: Theme.danger }]}
                    >
                      {currencySymbol}
                      {(stats?.totalCredit || 0).toFixed(2)}
                    </Text>
                  </View>
                  <View style={styles.reportSummaryItem}>
              ...
              {recentCollections.length === 0 ? (
                ...
                recentCollections.map((col) => {
                  const formattedColDate =
                    formatToSingaporeDate(col.CreatedDate, {
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                    }) +
                    " " +
                    formatToSingaporeTime(col.CreatedDate, {
                      hour: "2-digit",
                      minute: "2-digit",
                      hour12: false,
                    });
                  return (
                    <View
                      ...
                      style={styles.collectionRowCard}
                    >
                      <View style={{ flex: 1 }}>
                        <Text style={styles.collectionCustomerName}>
                          {col.CustomerName}
                        </Text>
                        <Text style={styles.collectionCustomerType}>
                          {col.CustomerType} • {col.PaymentMethod || "CASH"}
                        </Text>
                        {col.Remarks && (
                          <Text style={styles.collectionRemarks}>
                            {col.Remarks}
                          </Text>
                        )}
          ...
          <View style={styles.modalOverlay}>
            ...
            >
                ...
                <View style={{ flex: 1, minHeight: 0 }}>
                  ...
                  <View style={styles.ledgerTabs}>
                    <TouchableOpacity
                      style={[
                        styles.ledgerTabBtn,
                        ledgerTab === "LEDGER" && styles.activeLedgerTabBtn,
                      ]}
                    ...
                    >
                      <Text
                        style={[
                          ...
                          ledgerTab === "LEDGER" && styles.activeLedgerTabText,
                    ...
                    <TouchableOpacity
                      style={[
                        styles.ledgerTabBtn,
                        ledgerTab === "BILLS" && styles.activeLedgerTabBtn,
                      ]}
                    ...
                    >
                      <Text
                        style={[
                          styles.ledgerTabText,
                          ledgerTab === "BILLS" && styles.activeLedgerTabText,
                          isMobile && { fontSize: 11 },
                        ]}
                      >
                  ...
                  <ScrollView
                    style={{ flex: 1, padding: isMobile ? 12 : 20 }}
                    showsVerticalScrollIndicator={false}
                  >
                    {ledgerTab === "LEDGER" ? (
                        ...
                        {transactions.length === 0 ? (
                          ...
                          <View style={styles.ledgerTable}>
                            ...
                            <View style={styles.ledgerRowHeader}>
                              <Text
                                style={[styles.ledgerColHeader, { flex: 1.1 }]}
                                numberOfLines={1}
                              ...
                              <Text
                                style={[styles.ledgerColHeader, { flex: 1.2 }]}
                                numberOfLines={1}
                                adjustsFontSizeToFit
                              >
                                REF/REMARKS
                            ...
                            {transactions.map((tx, idx) => {
                              const formattedDate =
                                formatToSingaporeDate(tx.CreatedDate, {
                                  day: "numeric",
                                  month: "short",
                                }) +
                                " " +
                                formatToSingaporeTime(tx.CreatedDate, {
                                  hour: "2-digit",
                                  minute: "2-digit",
                                  hour12: false,
                                });
                        ...
                        {outstandingBills.length === 0 ? (
                          ...
                          <View style={{ gap: 10 }}>
                            {outstandingBills.map((bill) => {
                              const formattedBillDate = formatToSingaporeDate(
                                bill.InvoiceDate,
                                {
                                  day: "numeric",
                                  month: "short",
                              ...
                              return (
                                <View
                                  key={bill.SettlementId}
                                  style={styles.billItemCard}
                                >
                                  <TouchableOpacity
                                    activeOpacity={0.8}
                                    onPress={() =>
                                      handleToggleBillExpansion(
                                        bill.TransactionId,
                                      )
                                    }
                                    style={{
                                  ...
                                  >
                                    ...
                                    >
                                      <View style={styles.billItemBadge}>
                                        <Text style={styles.billItemBadgeText}>
                                          DUE
                                        </Text>
                                      </View>
                                      <Ionicons
                                  ...
                                  {bill.OutstandingAmount > 0.01 && (
                                    ...
                                    >
                                      <Ionicons
                                        ...
                                        color="#fff"
                                      />
                                      <Text style={styles.payBillInFullText}>
                                        Pay Bill in Full
                                      </Text>
                                    </TouchableOpacity>
                                  )}
                                  ...
                                  {expandedBillId === bill.TransactionId && (
                                    <View
                                      style={styles.billSettlementsContainer}
                                    >
                                      <Text style={styles.billSettlementsTitle}>
                                        Settlement History
                                      </Text>
                                      {loadingSettlements &&
                                      !billSettlements[bill.TransactionId] ? (
                                        <ActivityIndicator
                                          size="small"
                                          color={Theme.primary}
                                        ...
                                        <Text
                                          style={styles.billSettlementsEmpty}
                                        >
                                        ...
                                        (
                                          ...
                                          return (
                                            <View
                                              key={setl.AllocationId}
                                              style={styles.billSettlementRow}
                                            >
                                              <View style={{ flex: 1 }}>
                                                <Text
                                                  style={
                                                    styles.billSettlementMethod
                                                  }
                                                >
                                                  {setl.PaymentMethod || "CASH"}{" "}
                                                  {setl.ReferenceNo
                                                    ? `(${setl.ReferenceNo})`
                                                    : ""}
                                                </Text>
                                              ...
                                              >
                                                <Text
                                                  style={
                                                    styles.billSettlementAmount
                                                  }
                                                >
                                                  {currencySymbol}
                                                  {setl.AllocatedAmount.toFixed(
                                                    2,
                                                  )}
                                                </Text>
                                              </View>
              ...
              {!loadingDetails && (
                <View
                  onLayout={(e) =>
                    setFooterDiagHeight(e.nativeEvent.layout.height)
                  }
                ...
                >
                  ...
                  <TouchableOpacity
                    ...
                    disabled={
                      !selectedCustomer ||
                      selectedCustomer.OutstandingBalance <= 0.01
                    }
                  >
        ...
        <Modal visible={modalMode !== "NONE"} transparent animationType="slide">
          <View style={styles.modalOverlay}>
            <View style={[styles.formSheet, { maxHeight: "90%", padding: 0 }]}>
              <View style={styles.sheetHeader}>
                    ...
                    <View style={styles.inputGroup}>
                      ...
                      <View style={{ flexDirection: "row", gap: 10 }}>
                        <TouchableOpacity
                          style={[
                            styles.statusToggle,
                            formData.isActive && styles.activeToggle,
                          ]}
                          onPress={() =>
                        ...
                        >
                          <Text
                            style={[
                              styles.statusText,
                              formData.isActive && styles.activeStatusText,
                            ]}
                          >
                            Active
                          </Text>
                        ...
                        <TouchableOpacity
                          style={[
                            styles.statusToggle,
                            !formData.isActive && styles.inactiveToggle,
                          ]}
                          onPress={() =>
                        ...
                        >
                          <Text
                            style={[
                              styles.statusText,
                              !formData.isActive && styles.inactiveStatusText,
                            ]}
                          >
                            Inactive
                          </Text>
                ...
                <View style={{ flexDirection: "row", gap: 10 }}>
                  <View style={{ flex: 1 }}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(v) =>
                        setFormData({ ...formData, creditLimit: v })
                      }
                    />
                  </View>
                  {modalMode !== "EDIT" && (
                    <View style={{ flex: 1 }}>
                      ...
                      <TextInput
                        ...
                        onChangeText={(v) =>
                          setFormData({ ...formData, currentBalance: v })
                        }
                      />
                    </View>
                  )}
        ...
        >
          ...
          >
            <View
              style={[styles.formSheet, { maxHeight: "60%", width: "80%" }]}
            >
              <View style={styles.sheetHeader}>
...
const styles = StyleSheet.create({
  ...
  billSettlementsContainer: {
    marginTop: 12,
    borderTopWidth: 1,
    borderTopColor: Theme.border,
    paddingTop: 10,
  },
  billSettlementsTitle: {
    fontSize: 11,
    fontFamily: Fonts.bold,
    color: Theme.textPrimary,
    marginBottom: 8,
  },
  billSettlementsEmpty: {
    fontSize: 10,
    fontFamily: Fonts.medium,
    color: Theme.textMuted,
    fontStyle: "italic",
